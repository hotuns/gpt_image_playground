  ...
  const commitSettings = (nextDraft: AppSettings) => {
    const normalizedProfiles = nextDraft.profiles.map((profile) => {
      ...
      return {
        ...
        chatModel: profile.chatModel.trim() || DEFAULT_RESPONSES_MODEL,
            ...
            {activeTab === 'api' && (
              ...
              {activeProfile.provider === 'openai' && (
                <label className="block">
                  ...
                  <input
                    value={activeProfile.chatModel}
                    onChange={(e) => updateActiveProfile({ chatModel: e.target.value })}
                    onBlur={(e) => commitActiveProfilePatch({ chatModel: e.target.value })}
                    type="text"
                    placeholder={DEFAULT_RESPONSES_MODEL}
                    className="w-full rounded-xl border border-gray-200/70 bg-white/60 px-3 py-2.5 text-sm text-gray-700 outline-none transition focus:border-blue-300 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-200 dark:focus:border-blue-500/50"
                  />
                  <div data-selectable-text className="mt-1.5 text-xs text-gray-500 dark:text-gray-500">
                    Chat 固定走 <code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-white/[0.06]">/v1/responses</code>，与上方图片接口的 <code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-white/[0.06]">apiMode</code> 无关。默认使用 <code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-white/[0.06]">{DEFAULT_RESPONSES_MODEL}</code>。
                  </div>
                </label>
              )}
              ...
              <div className="space-y-4">
                <div className="rounded-2xl bg-blue-50/50 p-4 border border-blue-100/50 dark:bg-blue-500/5 dark:border-blue-500/10 flex items-start gap-3">
                  ...
                  <div className="text-[13px] leading-relaxed text-blue-800/80 dark:text-blue-300/80">
                    所有的配置、任务记录、Chat 会话和生成的图片均仅保存在您的浏览器本地（除非您使用的服务商存储了它们）。如果您需要清理浏览器站点数据、重置浏览器或使用其他设备，请先导出备份。
                  </div>
                </div>
                ...
                <div className="rounded-2xl border border-gray-100 bg-white p-4 dark:border-white/[0.06] dark:bg-white/[0.02] space-y-4 shadow-sm">
                  ...
                  <div className="flex flex-wrap gap-x-6 gap-y-3">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer group">
                      ...
                      <span className="text-[13px] font-medium text-gray-700 dark:text-gray-200 group-hover:text-gray-900 dark:group-hover:text-white transition-colors">包含任务、图片和 Chat</span>
                    </label>
                  </div>
                  <button
